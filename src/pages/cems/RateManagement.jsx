  ...
  return (
    <div>
      <DevNote
        items={[
          '"외국인 웹사이트" 행이 새로 추가됨',
          '"외국인 웹사이트" 매입 환율을 직접 입력하면 외국인 웹사이트 신청화면(STEP B)·홈 티커에 실시간 반영됨(수동 고정). "자동" 버튼으로 2분 주기 변동으로 복귀',
          '환율은 2분마다 자동 소폭 변동(±0.1~0.5%) — 프로토타입 시뮬레이션(실제 환율 API 아님). 소액통화(VND 등)도 유효숫자 반올림으로 정상 변동',
          '다른 채널(지점/환전예약/온라인환전)의 매각·매입 수동/자동(%) 설정은 화면 상태 데모일 뿐 STEP B에 연동되지 않음',
        ]}
      />
