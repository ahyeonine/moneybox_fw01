...
export default function LookupPage() {
  ...
  return (
    <div>
      <DevNote
        items={[
          '조회 조건: 예약번호 + 이메일 (전화번호 없음). 조회 시 같은 이메일의 모든 예약을 리스트로 보여주고, 행 클릭 시 상세 진입',
          '"예약(BOOKED)" 상태일 때만 취소/변경 가능. 완료·취소 건은 안내만 노출',
          '가용시재 차감(예약시재 반영)은 예약완료가 아니라 여기 "방문 예정 확인" 시점에 발생 — 이 시점에 재고 재확인(동시성). 데모: rush@example.com (B004+USD 재고 1개, 예약 2건)',
          '방문예정확인 후 미방문으로 수령기한(당일 KST 자정) 경과 시 → 자동취소되며, 확인시점에 잡았던 재고는 복구됨(무응답 자동취소는 미반영이라 복구 없음)',
          '변경(날짜/지점/통화/금액) 시 환율은 현재 공유 환율로 재계산됨',
          '고객 직접취소(cancelReason=CUSTOMER)는 노쇼 카운트에서 제외. CEMS엔 "취소"로 표시, POS 목록에선 제외',
        ]}
      />
