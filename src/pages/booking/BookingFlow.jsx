...
const DEV_NOTES = {
  apply: [
    ...
    '최소미만 입력도 하드블록이 아니라 blur 시 최소값으로 자동보정(정책: "최소미만시 최소로"). 하드블록 아님에 유의',
    '최소/최대 한도는 CEMS "외국인서비스 한도관리" 공유 상태에서 읽어옴(하드코딩 아님) — 관리자 변경 즉시 반영',
    '환율은 공유 상태(2분마다 자동 변동 + CEMS "외국인 웹사이트" 수동값)에서 읽음. 값 변경 시 플래시로 표시되며 "신청하기" 클릭 시점 값으로 픽스',
    '수령일 선택 최대 범위: 리드타임 이후 ~ 2주 이내 (지점별 leadTimeDays 상이)',
    '재고 소진 데모: 인천공항 T1(B003)+VND 조합은 결정적으로 소진 처리(isSoldOut). 실제 재고 API 아님',
    '기존 외화구매/외화판매 두 개 탭이었으나, 현재는 원화구매만 남음 (외화구매 탭 제거됨)',
  ],
  info: [
    '필수 항목은 예약자명 + 이메일만. 메신저ID, 휴대전화, 생년월일 수집 안 함 (온라인 최소수집 원칙)',
    '노쇼 자동취소(cancelReason=AUTO)가 이메일 기준 NOSHOW_LIMIT(현재 2, TODO) 이상 누적되면 이 단계에서 신규예약 차단(실시간 카운트). 데모: noshow@example.com',
  ],
  consent: [
    '클릭 시 펼쳐지는 약관 전문은 프로토타입용 더미 텍스트, 실제 법무 검토 문구 아님',
    '취소 컷오프 정책은 미확정(TODO) — 화면에 "언제든 취소"·"컷오프 없음" 등 확정적 표현은 노출하지 않음',
  ],
  done: [
    '예약 완료 즉시 상태값 "예약(BOOKED)"으로 저장됨 — 이 시점엔 재고 미반영(재고는 리마인더 "방문 예정" 확인 시 차감)',
    '예약번호 채번: RSV-YYYYMMDD-#### (기준일 + 순번). 프로토타입은 in-memory 채번이라 새로고침 시 초기화',
    '완료 즉시 CEMS "외국인 환전예약관리" 리스트와 POS "전체보기"에 반영(단일 공유 store)',
  ],
}
