...
// 지점 어드민(CEMS) 한도관리용 프론트 상태.
// CEMS는 각 지점이 로그인해 쓰는 어드민이므로, 이 상태는 "현재 로그인 지점"의 설정을 뜻한다.
// (프로토타입은 지점 계정 분리를 시뮬레이션하지 않아 단일 스토어로 한 지점을 대표)
// - minAmounts: 통화별 최소 환전금액 (지점 설정)
// - maxAmounts: 통화별 건당 최대 환전금액 (지점 설정) { currency: number }
// 실제 저장 API 없음. 새로고침 시 초기값으로 리셋.

const SettingsContext = createContext(null)

// 건당 최대금액 초기 시드: 대표 지점(첫 지점) 목데이터의 통화별 max 값을 불러온다.
// (로그인 지점 기준 값 — 실제로는 지점마다 별도 저장)
function seedMax() {
  const out = {}
...
export function SettingsProvider({ children }) {
  ...
  // 통화별 신청 단위 (지점 설정). 외국인 웹사이트 신청화면에서만 올림 적용.
