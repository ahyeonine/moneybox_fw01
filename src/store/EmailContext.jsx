...
// 비회원 신청완료 메일에 넣는 회원가입 유도 문구 — {{signupHook}} 치환값
// (회원 예약은 빈 값으로 치환되어 노출되지 않음. 프로모션은 기간 제한 없이 상시)
export const SIGNUP_HOOK =
  '아직 회원이 아니신가요? 지금 회원가입하면 회원 우대 환율 + 할인 쿠폰 · 환율 알림 혜택을 드려요. ' +
  '다음 예약부터는 정보 입력 없이 바로 예약하실 수 있습니다. → [회원가입]\n\n'
  ...
  return {
    ...
    applied: {
      ...
      body:
        ...
        '- 적용 환율: 수령일 지점 전광판 환율(예약 시 미고정)\n' +
        '- 수령 예정일: {{pickupDate}}\n\n' +
        '환율은 수령일 지점 전광판 기준으로 적용돼요. 방문하시는 날 환율이 더 좋아졌다면, 그날의 더 나은 환율로 적용해 드립니다.\n\n' +
