...
- 취소 응답·무응답으로 인한 자동취소는 아직 차감 전(미확정)이거나 방문 실패 케이스이므로 별도 **재고 복구 로직은
  불필요**하다.
- 프로토타입: `B004+USD` 재고 1개 + `rush@example.com`으로 조회되는 방문예정 미확인 예약 2건으로 경쟁 상황을
  시뮬레이션(첫 건 확인 성공 → 재고 1→0, 둘째 건 확인 시 소진 안내).
