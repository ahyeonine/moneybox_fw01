...
- 다국어: 한국어 / 영어
- 수령방식: 지점수령 단일 · 결제: 무결제 예약 + 현장 전액결제 · 회원/로그인/온라인 본인인증 없음(이메일 OTP는 이메일 수신 가능 여부 확인용) · 수령 시 지점 신분증 대조는 기존 POS 흐름
...
| ↳ `/pos/transaction` | 직원 | 거래처리 (예약번호 조회 → 신분증 대조 → 거래완료) + 시뮬레이션, `홈으로` 복귀 |
